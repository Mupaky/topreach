...
// AdminLayout is no longer imported/used here, it's in the parent Server Component
import { createClient } from "@/utils/client";
import { PlusCircle, Edit, Trash2, Save, XCircle } from "lucide-react";


const supabase = createClient(); // This client is for fetching packages client-side
...
export default function AdminPackagesClient({ initialUser, initialPackages, fetchErrorMsg }) {
    ...
    useEffect(() => {
        ...
        // If data was passed from server, we are not initially loading on client
        if (initialPackages || fetchErrorMsg) {
            setIsLoadingPackages(false);
        }
    }, [initialUser, initialPackages, fetchErrorMsg]);
    ...
    // --- API Interaction Functions (handleDeletePackage, updatePackage, handleAddPackage) ---
    // These remain IDENTICAL to your previous version of AdminPackagesPage.jsx
    // ... (paste your handleDeletePackage, updatePackage, handleAddPackage functions here) ...
    ...
    const handlePackageDetailChange = (packageId, field, value) => {
        ...
        if (field !== 'price') {
            ...
            if (isNaN(parsedValue)) parsedValue = field === 'lifespan' ? 1 : 0;
        } else {
    ...
    // If not an admin (based on prop), render an access denied message.
    // This client-side check is a secondary measure. The server component handles the primary redirect.
    if (!isClientMounted) {
        return <div className="text-white text-center p-10">Инициализиране на страницата...</div>; // Or null
    }
    if (!user || user.role !== 'admin') {
        // This should ideally be caught by the server component redirect,
        // but as a fallback client-side display.
        return (
    ...
    if (error && !packages.length) { // Show persistent fetch error if no packages loaded
        return (
            <div className="text-center py-10">
    ...
    // --- JSX Render (remains IDENTICAL to your previous AdminPackagesPage JSX) ---
    return (
        <div className="space-y-10"> {/* AdminLayout is now provided by the parent Server Component */}
            ...
            <div className="bg-gray-800 p-6 rounded-xl shadow-xl border border-gray-700">
                ...
                ) : !error ? ( // if no error and no packages
